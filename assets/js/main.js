    ...
    sortMainCategories(mainCategories).forEach(main => {
        ...
        let getFilteredItems = (itemsList, sub) => {
            ...
            sortItems(itemsList).forEach(item => {
                ...
                filtered.push({item, sub, sizeName, available, inOrder});
            });
            return filtered;
        };
                ...
                if (visibleItems.length > 0) {
                    ...
                    visibleItems.forEach((v, idx) => {
                        ...
                        let packingColHtml = isFitting 
                            ? `<td style="padding: 0.2rem 0.5rem; border-bottom: ${borderBottom}; text-align:center; font-weight:600; color: #000; border-right: 1px solid #000; white-space: nowrap;">${pckQtyStr}</td>
                               <td style="padding: 0.2rem 0.5rem; border-bottom: ${borderBottom}; text-align:center; font-weight:600; color: #000; border-right: 1px solid #000; white-space: nowrap;">${packingStr}</td>`
                            : '';
                        ...
                        itemsHtml += `
                                    <tr style="background: white;">
                                        ...
                                        ${packingColHtml}
                                        <td style="padding: 0.2rem 0.5rem; border-bottom: ${borderBottom}; text-align:center; border-right: 1px solid #000; color: #000; white-space: nowrap;">${displayLengthOrWeight}</td>
...
function updateBrandAuditTotals(brandId) {
    ...
    brandGroup.querySelectorAll('tbody tr').forEach(row => {
        ...
        if (sysPcsEl && gdPcsEl) {
            ...
            let dPcs = parseInt(diffPcsEl?.textContent.replace('+', '')) || 0;
            let dKg = parseFloat(diffKgEl?.textContent.replace('+', '')) || 0;
            
            diffTotalPcs += dPcs;
            diffTotalKg += dKg;
            
            if (dPcs > 0) excessTotalPcs += dPcs;
            if (dPcs < 0) shortageTotalPcs += dPcs;
            if (dKg > 0) excessTotalKg += dKg;
            if (dKg < 0) shortageTotalKg += dKg;
        }
    });
    ...
    if (totDiffPcs) {
        totDiffPcs.textContent = diffTotalPcs > 0 ? '+' + diffTotalPcs : diffTotalPcs;
        totDiffPcs.className = diffTotalPcs > 0 ? 'diff-plus' : (diffTotalPcs < 0 ? 'diff-minus' : '');
    }
    if (totDiffKg) {
        totDiffKg.textContent = diffTotalKg > 0 ? '+' + diffTotalKg.toFixed(2) : diffTotalKg.toFixed(2);
        totDiffKg.className = diffTotalKg > 0 ? 'diff-plus' : (diffTotalKg < 0 ? 'diff-minus' : '');
    }
...
function updateGrandAuditTotal() {
    let totals = {
        Pipe: { sysPcs: 0, sysKg: 0, gdPcs: 0, gdKg: 0, diffPcs: 0, diffKg: 0, exPcs: 0, exKg: 0, shPcs: 0, shKg: 0 },
        Fitting: { sysPcs: 0, sysKg: 0, gdPcs: 0, gdKg: 0, diffPcs: 0, diffKg: 0, exPcs: 0, exKg: 0, shPcs: 0, shKg: 0 }
    };
    ...
    mainCategories.forEach(m => {
        ...
        const diffKgText = document.getElementById(`totalDiffKg_${m.id}`)?.textContent || '0.00';
        ...
        totals[type].diffPcs += diffPcs;
        totals[type].diffKg += diffKg;
...
function refreshRMDashboard() {
    ...
    rmItems.forEach(item => {
        ...
        summaryHtml += `
            <div style="background: #f0f9ff; border: 1px solid #000; padding: 8px 12px; border-radius: 6px; display: flex; justify-content: space-between; align-items: center;">
                <div>
